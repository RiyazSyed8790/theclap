...
export default function Movies_Page(){
 ...
 function postQuery(){
   axios.get(// `http://localhost:3000/query/${query}` || for dev
 ...
 React.useEffect(function(){
   setId(window.location.pathname.split("/")[1]);
    axios.get(// `http://localhost:3000/users/${id}` || for dev
    ...
    axios.get( // `http://localhost:3000/` || for dev
   ...
   setTimeout(() => {
    ...
    return(
        <main className=" min-h-screen  md:max-w-screen bg-[#10141E] text-white">
           ...
           <Searchbar place={"TV series"} query={query} change={handleSearch} click={postQuery} />
           <section className=" relative py-28 md:py-20 md:pl-[164px] pl-4 flex flex-col">
            <div className="Recommended">
               <h2 className=" text-3xl outfit font-light">TV Series</h2>
